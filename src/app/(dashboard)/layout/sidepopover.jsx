...
const SidePopover = () => {
  ...
  const displayName = lenzrouser?.displayName || lenzrouser?.email || "User";
  const initials = getInitials(
    lenzrouser?.displayName || lenzrouser?.email || "U",
  );
  return (
    <div className="w-70 rounded-xl shadow-2xl border border-zinc-800 bg-black py-2 font-sans animate-in fade-in zoom-in-95 duration-200 max-h-[92vh] overflow-y-auto scrollbar-thin scrollbar-pill">
      ...
      <div className="px-1 space-y-0.5 pb-2 mb-2">
        ...
        <MenuAction icon={<Settings size={18} />} label="Settings" />
      ...
      <div className="px-1 space-y-0.5">
        ...
        <MenuAction icon={<LogOut size={18} />} label="Log out" />
      </div>
    </div>
  );
};

const MenuAction = ({ icon, label, extra, showPin, isActive }) => {
  return (
    <button
